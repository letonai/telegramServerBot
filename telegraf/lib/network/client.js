...
var proxy = 'socks://127.0.0.1:5555';
console.log('using proxy server %j', proxy);
var agentSocks = new SocksProxyAgent(proxy, true);
var endpoint = 'https://api.telegram.org';
console.log('attempting to GET %j', endpoint);
var opts = url.parse(endpoint);
opts.agent = agentSocks;
...
const defaultOptions = {
  ...
  agent: new https.Agent({
	...
	//proxy: "http://a71278:EuOdeioSenhas%23%40!!%40%23@ncproxy1:8080"
  })
}
...
class ApiClient {
  constructor (token, options, webhookResponse) {
    this.token = token
    this.options = Object.assign({}, defaultOptions, options,opts)
